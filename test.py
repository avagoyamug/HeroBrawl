'''
TODO:
working in hero list assembly and intro function
'''
...
def InitMove(direction):
    ...
    if heroTeam[turnToggler][activeNo].dash > 0:
        print('Dashing')
        newPos, animPos = Dash(direction, pos, heroTeam[turnToggler][activeNo].dash)
    else:
        newPos, animPos = Move(direction, pos)
    ...
    if newPos in heroPos[not turnToggler]: #ATTACK
        print()
        print('ATTACK!')
        Attack(oldPos, newPos, direction)
        return True
    elif newPos in heroPos[turnToggler]: #checks collision
        print('InitMove function detected collision with FRIEND, Movement canceled')
        heroPos[turnToggler][activeNo] = oldPos
...
def gameIntro():
    ...
    # connecting hero classes to spawned heroes
    ''' For now, choose in Terminal; building UI char-picker in parallel and connecting in the end 
    for i in range(heroCount):
        n = {1 : char.Dwarf(i), 2 : char.Elf(i), 3 : char.Daemon(i), 4 : char.Berserk(i)}
        heroTeam[0].append(n[int(input('TEAM 1, please choose character: '))])

    for i in range(enemyCount):
        n = {1 : char.Dwarf(i), 2 : char.Elf(i), 3 : char.Daemon(i), 4 : char.Berserk(i)}
        heroTeam[1].append(n[int(input('TEAM 2, please choose character: '))])
    ...
    while intro == True:
        ...
        if keys[pygame.K_1]:
            if inputCooldownCounter == 0:
                if len(heroTeam[0]) < heroCount:
                    heroTeam[0].append(char.Dwarf(count))
                elif len(heroTeam[1]) < enemyCount:
                    heroTeam[1].append(char.Dwarf(count))
        ...
        if keys[pygame.K_2]:
            if inputCooldownCounter == 0:
                if len(heroTeam[0]) < heroCount:
                    heroTeam[0].append(char.Elf(count))
                elif len(heroTeam[1]) < enemyCount:
                    heroTeam[1].append(char.Elf(count))
        ...
        if keys[pygame.K_3]:
            if inputCooldownCounter == 0:
                if len(heroTeam[0]) < heroCount:
                    heroTeam[0].append(char.Daemon(count))
                elif len(heroTeam[1]) < enemyCount:
                    heroTeam[1].append(char.Daemon(count))
        ...
        if keys[pygame.K_4]:
            if inputCooldownCounter == 0:
                if len(heroTeam[0]) < heroCount:
                    heroTeam[0].append(char.Berserk(count))
                elif len(heroTeam[1]) < enemyCount:
                    heroTeam[1].append(char.Berserk(count))
